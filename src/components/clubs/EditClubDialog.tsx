...
export function EditClubDialog({ open, onOpenChange, club }: EditClubDialogProps) {
  ...
  useEffect(() => {
    if (club) {
      setFormData({
        name: club.name,
        discount_type: club.discount_type,
        discount_value: club.discount_value.toString(),
        coupon_code: club.coupon_code,
      });
    }
  }, [club]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[600px] p-0 bg-white rounded-[30px] border-none [&>button]:hidden">
        <div className="flex items-center justify-between p-6 border-b border-[#742551]/20">
          <h2 className="text-[22px] font-normal text-[#742551]">עריכת מועדון קיים</h2>
        ...
        <div className="p-6 pt-0 flex justify-end">
          ...
          >
            ...
            <span>{updateClub.isPending ? 'שומר...' : 'עדכון ושמירה'}</span>
          </button>
        </div>
      </DialogContent>
    </Dialog>
